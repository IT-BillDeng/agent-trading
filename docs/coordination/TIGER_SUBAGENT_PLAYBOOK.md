...
- `tiger-watcher`：运行健康 / 权限 / 日志 / 队列 / 周期输出监控
- `tiger-newswire`：新闻、事件、催化与时间线整理
- `tiger-strategist`：策略、风险收益、仓位与计划建议
- `tiger-executor`：执行步骤、参数检查、preview / dispatch 校验
- `tiger-scout`：扫描候选标的、异常波动与待验证机会
- `tiger-closer`：退出、止盈止损、减仓平仓与收尾条件建议
...
调用：`tiger-watcher`
适合：
- 盯 `tiger-trading/shared/tiger_shared_watchlist.json` 中 `enabled=true` 的标的
- 看 BUY / EXIT 候选是否变化
...
1. `tiger-scout` / `tiger-newswire` 先收集
2. `tiger-strategist` 形成计划
3. `tiger-executor` 拆执行步骤
4. `tiger-watcher` 做运行与结果复核
5. `tiger-closer` 负责退出与收尾
