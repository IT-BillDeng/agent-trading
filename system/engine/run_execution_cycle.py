...
from engine.runtime import run_execution_cycle
from engine.tiger_client import TigerClient as DefaultBrokerClient


def main() -> int:
    if len(sys.argv) != 3:
        print('usage: python3 run_execution_cycle.py <app_config.json> <broker_props.properties>')
        return 2

    app = load_app_config(sys.argv[1])
    props = load_tiger_props(sys.argv[2])
    client = DefaultBrokerClient(props)
    summary = run_execution_cycle(client, app)
