...
class FactorResearcherStructureTests(unittest.TestCase):
    def test_factor_researcher_agent_yaml_exists_and_has_safe_write_scope(self):
        ...
        for forbidden_fragment in (
            "/rules/",
            "/workspace/agent-trading/factors/",
            "/runtime/",
            "/logs/latest/",
            "/artifacts/broker/",
            "execution.py",
    ...
    def test_factor_research_taskfile_contains_no_submit_no_apply_no_secrets_constraints(self):
        ...
        self.assertIn("./artifacts/factor_research/reports/latest.md", text)
        self.assertIn("./artifacts/factor_research/hypotheses.jsonl", text)
        self.assertIn("./artifacts/factor_research/dual_run/latest.json", text)
        self.assertIn("./artifacts/factor_research/dual_run/reports/latest.md", text)
        self.assertIn("./artifacts/factor_research/dual_run/observations/latest.json", text)
        self.assertIn("./artifacts/factor_research/facts/latest.json", text)
        self.assertIn("./artifacts/factor_research/facts/history.jsonl", text)
        self.assertIn("./artifacts/factor_research/facts/reports/latest.md", text)
        self.assertIn("./artifacts/factor_research/scenarios/latest.json", text)
        self.assertIn("./artifacts/factor_research/scenarios/history.jsonl", text)
        self.assertIn("./artifacts/factor_research/scenarios/reports/latest.md", text)
        self.assertIn("python -m engine.factors.research", text)
        self.assertIn("python -m engine.factors.sample --mode live_snapshot", text)
        self.assertIn("python -m engine.factors.sample --mode historical_backfill", text)
        self.assertIn("python -m engine.factors.facts collect", text)
        self.assertIn("python -m engine.factors.facts summarize", text)
        self.assertIn("python -m engine.factors.parity", text)
        self.assertIn("scripts/dev/run-factor-dual-run-observation.sh", text)
        self.assertIn("scripts/dev/run-factor-dual-run-real-cycle.sh", text)
        self.assertIn("--sample-source backfill", text)
        self.assertIn("AGENT_TRADING_DASHBOARD_URL=http://host.docker.internal:18088", text)
        self.assertIn("DASHBOARD_BASE_URL", text)
        self.assertIn("factor_sample_health", text)
        self.assertIn("live_observation_count", text)
        self.assertIn("backfill_observation_count", text)
        self.assertIn("insufficient_labeled_samples", text)
        self.assertIn("labeled_sample_count", text)
        self.assertIn("candidate_grade", text)
        self.assertIn("no-lookahead validation status", text)
        self.assertIn("factor-parity-contract.md", text)
        self.assertIn("blocking_mismatch_count", text)
        self.assertIn("parity pass 不等于 actionable", text)
        self.assertIn("factor_dual_run_summary", text)
        self.assertIn("dual-run pass 不等于 factor actionable", text)
        self.assertIn("production signals unchanged", text)
        self.assertIn("diagnostic_factor_rules.json", text)
        self.assertIn("diagnostic_only=true", text)
        self.assertIn("production_rules_modified=false", text)
        self.assertIn("actionable_enabled=false", text)
        self.assertIn("factor_rule_link", text)
        self.assertIn("approval_decisions.jsonl", text)
        self.assertIn("human", text)
        self.assertIn("main_agent", text)
        self.assertIn("docs/approval-integrity-contract.md", text)
        self.assertIn("docs/historical-fact-replay-contract.md", text)
        self.assertIn("historical_fact_summary", text)
        self.assertIn("leakage_warnings", text)
        self.assertIn("fact replay 不等于 actionable factor", text)
        self.assertIn("readiness_status", text)
        self.assertIn("readiness_reasons", text)
        self.assertIn("synthetic_fixture_only", text)
        self.assertIn("status=draft", text)
        self.assertIn("apply_allowed=false", text)
