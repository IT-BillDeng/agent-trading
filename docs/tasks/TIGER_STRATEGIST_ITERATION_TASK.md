...
1. 每次策略变更必须回测验证
2. 新方案必须优于基线才上线
3. 每次尝试都记录到 strategist_iterations/
...
# 读取最近迭代结果
ls runtime/tiger_engine/strategist_iterations/
cat runtime/tiger_engine/strategist_iterations/latest.json
```
...
### Step 7: 记录迭代
迭代结果自动保存到 `strategist_iterations/iter_YYYYMMDD_HHMMSS.json`
