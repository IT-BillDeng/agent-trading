...
class SafeHandoffScriptTests(unittest.TestCase):
    def test_make_safe_handoff_creates_filtered_zip(self):
        ...
        (repo_root / "factors").mkdir(parents=True)
        (repo_root / "factors" / "registry.json").write_text("{}")
        (repo_root / "factors" / "README.md").write_text("factors")
        ...
        (repo_root / "config" / "broker_fee.tiger.json").write_text("{}")
        ...
        (repo_root / "scripts" / "make_safe_handoff.sh").write_text("#!/usr/bin/env bash\n")
        (repo_root / "specs").mkdir(parents=True)
        (repo_root / "specs" / "factor-registry-schema-v1.md").write_text("factor schema")
        (repo_root / "specs" / "strategist-output-schema-v1.md").write_text("schema")
        (repo_root / "system" / "engine").mkdir(parents=True, exist_ok=True)
        (repo_root / "system" / "engine" / "requirements.txt").write_text("fastapi\n")
        ...
        (repo_root / "tests" / "test_api.py").write_text("pass")
        ...
        (repo_root / "logs" / "latest" / "misc.json").write_text("{}")
        ...
        self.assertIn("Critical review files retained:", result.stdout)
        ...
        self.assertIn("factors/registry.json", names)
        self.assertIn("factors/README.md", names)
        ...
        self.assertIn("config/broker_fee.tiger.json", names)
        self.assertIn("agents/strategist.yaml", names)
        self.assertIn("cron/job.json", names)
        self.assertIn("scripts/make_safe_handoff.sh", names)
        self.assertIn("specs/factor-registry-schema-v1.md", names)
        self.assertIn("specs/strategist-output-schema-v1.md", names)
        self.assertIn("system/engine/requirements.txt", names)
        ...
        self.assertNotIn("logs/latest/misc.json", names)
